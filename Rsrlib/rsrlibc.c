...
int rsr_check_header(RSR_file *rf)
{
    ...
    if (rf->head->days_1900 < 31047UL) {
	rsr_err("RSR data start day must be after 1/1/85");
	return 0;
    }

    if (rf->head->days_1900 > 55000UL) {  /* 8/1/2050 */
	rsr_err("RSR data start day is sort of perposterous");
	return 0;
    }
	...
	if (n == 0 && !rf->head->band_on) {
	    rsr_err("Type 2 file has no allthetime, daytime, nor counters, and band is off");
	    return 0;
	}
	/*
...
RSR_rec *rsr_next_record(RSR_file *rf)
{
    ...
    */
	 ...
	 */
 	...
 	rec_n = (reclen == rf->rec_len_day) ? rf->rec_n_day : rf->rec_n_nite;

	if (!rf->skip_unpack)
	        rsr_unpack_record(rf, rec_n);
